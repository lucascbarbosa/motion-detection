...
for i = 1:numel(sinais_brutos_filenames)
    if ~sinais_brutos_filenames(i).isdir
        ...
        file_path_filtrado = fullfile(sinais_filtrados_dir, [name '_emg.csv']);
        csvwrite(file_path_filtrado, signal_f);
        ...
        file_path_filtrado = fullfile(sinais_filtrados_dir, [name '_acel.csv']);
        csvwrite(file_path_filtrado, signal_f_acel);
%         figure; % 7
%         subplot(2,1,1);
%         plot(t, signal_f);
